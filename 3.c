...
// Parallel implementation 1: Basic parallel for with collapse(2)
...
void conv2d_parallel_dynamic(
    ...
    #pragma omp parallel for collapse(2) schedule(dynamic, 8) proc_bind(close)
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            float sum = 0.0f;
            for (int ki = -pad_h; ki <= pad_h; ki++) {
...
// Parallel implementation 4: Single loop with manual work distribution
void conv2d_parallel_single_loop(
    float **f, int H, int W,
    float **g, int kH, int kW,
    float **output
) {
...
// Parallel implementation 5: Tiled approach
...
int main(int argc, char *argv[]) {
    ...
    // Parallel implementation 1: Basic collapse(2)
    ...
    printf("Parallel basic (collapse(2)) time: %.9f seconds (%.2fx speedup)\n", 
    ...
    // Parallel implementation 4: Single loop
    double start_par4 = omp_get_wtime();
    conv2d_parallel_single_loop(f, H, W, g, kH, kW, output_par4);
    double end_par4 = omp_get_wtime();
    double par4_time = end_par4 - start_par4;
    printf("Parallel single loop time: %.9f seconds (%.2fx speedup)\n", 
           par4_time, seq_time / par4_time);
    ...
    int fastest_idx = 1;
    const char* fastest_name = "Basic collapse(2)";
    ...
    if (par4_time < fastest_time) { fastest_time = par4_time; fastest_idx = 4; fastest_name = "Single loop"; }
